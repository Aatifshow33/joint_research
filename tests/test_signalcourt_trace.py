...
def test_wallet_flow_pipeline_trace_blocked_and_has_expected_steps() -> None:
    ...
    assert [step.step_name for step in trace.steps] == EXPECTED_STEP_NAMES
    assert trace.blocked is True
    assert trace.paper_order_allowed is False
    assert trace.live_order_allowed is False
    assert trace_allows_order(trace) is False
    assert any(step.blockers for step in trace.steps)
    assert any(step.source_artifacts for step in trace.steps)
    assert trace.non_authorization_notice
    assert trace.warnings


def test_derivatives_pipeline_trace_blocked_and_has_expected_steps() -> None:
...
def test_trace_output_is_deterministic() -> None:
    first = build_trace_from_pipeline_result(
        build_derivatives_regime_pipeline(
            results_csv_path=DERIVATIVES_RESULTS_CSV,
            candidates_json_path=DERIVATIVES_CANDIDATES_JSON,
            summary_md_path=DERIVATIVES_SUMMARY_MD,
    ...
    second = build_trace_from_pipeline_result(
        build_derivatives_regime_pipeline(
            results_csv_path=DERIVATIVES_RESULTS_CSV,
            candidates_json_path=DERIVATIVES_CANDIDATES_JSON,
            summary_md_path=DERIVATIVES_SUMMARY_MD,
...
def test_no_output_files_are_written(tmp_path: Path) -> None:
    ...
    wallet_trace = build_trace_from_pipeline_result(
        build_wallet_flow_pipeline(
    ...
    derivatives_trace = build_trace_from_pipeline_result(
        build_derivatives_regime_pipeline(
