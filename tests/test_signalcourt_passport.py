...
def test_build_derivatives_regime_passport_from_committed_artifacts() -> None:
    ...
    assert passport.title == "Derivatives-Regime"
    assert passport.research_only is True
    assert passport.tradeable is False
    assert passport.promotion_blocked is True
    assert passport.trade_decision_blocked is True
    assert passport.simulation_ready_count == 0
    assert passport.watchlist_count == 0
    assert passport.weak_count == 158
    assert passport.rejected_count == 423
    assert "INSUFFICIENT_SAMPLES" in passport.top_blockers
    assert "TRAIN_TEST_DIRECTION_MISMATCH" in passport.top_blockers
    assert "NO_IMPROVEMENT_OVER_BASELINE" in passport.top_blockers
    assert "not tradeable" in passport.operator_warning.lower()
    assert "blocked" in passport.operator_warning.lower()
    assert passport_allows_trade_decision(passport) is False


def test_build_wallet_flow_passport_from_committed_artifacts() -> None:
    ...
    assert passport.title == "Wallet-Flow"
    assert passport.research_only is True
    assert passport.tradeable is False
    assert passport.promotion_blocked is True
    assert passport.trade_decision_blocked is True
    assert passport.simulation_ready_count == 0
    assert passport.watchlist_count == 0
    assert passport.weak_count == 0
    assert passport.rejected_count == 1082
    assert "exploratory" in passport.operator_warning.lower()
    assert "not tradeable" in passport.operator_warning.lower()
    assert passport.status == "EXPLORATORY_ONLY_NOT_TRADEABLE"
    assert passport_allows_trade_decision(passport) is False


def test_build_signal_passport_from_readiness_object() -> None:
    readiness = SignalReadiness(
        signal_id="custom_lane",
        lane="custom_lane",
        ...
        weak_count=3,
        rejected_count=7,
        top_blockers=["LOW_SAMPLE_COUNT"],
        source_artifacts=["/tmp/source.csv"],
        readiness_summary="custom summary",
        next_required_evidence=["add evidence"],
        non_authorization_notice="no auth",
    )

    passport = build_signal_passport(readiness)

    assert passport.signal_id == "custom_lane"
    assert passport.title == "Custom-Lane"
    assert passport.readiness_status == "RESEARCH_ONLY_BLOCKED"
    assert passport.top_blockers == ["LOW_SAMPLE_COUNT"]
    assert passport.next_required_evidence == ["add evidence"]
    assert passport.source_artifacts == ["/tmp/source.csv"]
...
def test_no_output_files_are_written(tmp_path: Path) -> None:
    ...
    _ = build_derivatives_regime_passport(
